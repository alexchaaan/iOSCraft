...
class CAssetDecoratedMap : public CTerrainMap{
    ...
    public:        
        ...
        CAssetDecoratedMap(const CAssetDecoratedMap &map, EPlayerColor newcolors[pcMax]);
        ~CAssetDecoratedMap();
        
        CAssetDecoratedMap &operator=(const CAssetDecoratedMap &map);
        
        static bool LoadMaps(std::shared_ptr< CDataSourceContainer > container);
        static int FindMapIndex(const std::string &name);
        static std::shared_ptr< const CAssetDecoratedMap > GetMap(int index);
        static std::shared_ptr< CAssetDecoratedMap > DuplicateMap(int index, EPlayerColor newcolors[pcMax]);
        ...
        std::weak_ptr< CPlayerAsset > FindNearestAsset(const CPosition &pos, EPlayerColor color, EAssetType type);
        bool CanPlaceAsset(const CPosition &pos, int size, std::shared_ptr< CPlayerAsset > ignoreasset);
        CPosition FindAssetPlacement(std::shared_ptr< CPlayerAsset > placeasset, std::shared_ptr< CPlayerAsset > fromasset, const CPosition &nexttiletarget);
        ...
        CPosition FindNearestReachableTileType(const CPosition &pos, ETileType type);
};
